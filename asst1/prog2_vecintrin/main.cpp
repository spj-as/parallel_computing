...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    ...
    // result = 1 for all valid lanes
    _cs149_vset_float(result, 1.f, maskAll);
    // active lanes: y > 0
    _cs149_vgt_int(maskActive, y, zeroInt, maskAll);

    while (_cs149_cntbits(maskActive) > 0) {
      // result *= x only for active lanes
      _cs149_vmult_float(result, result, x, maskActive);
      // y -= 1 only for active lanes
      _cs149_vsub_int(y, y, oneInt, maskActive);
      // update active mask
      _cs149_vgt_int(maskActive, y, zeroInt, maskAll);
    }

    // clamp result if result > 9.999999

    _cs149_vgt_float(maskClamp, result, maxVal, maskAll);
...
float arraySumVector(float* values, int N) {
  ...
  _cs149_vstore_float(tmp, sumVec, maskAll);
  float sum = 0.f;
